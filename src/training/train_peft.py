...
def run_experiment(
    ...
    # HYP=3.5 means the model is penalised 3.5× more for missing a HYP case
    # Values derived from class frequencies in PTB-XL
    pos_weight = torch.tensor([1.0, 1.8, 1.8, 1.9, 3.5]).to(device)
    criterion  = nn.BCEWithLogitsLoss(pos_weight=pos_weight)

    #  LR Scheduler
    # Warmup for 2 epochs then cosine decay
    # Warmup prevents large gradient updates early when adapter is random
    def lr_lambda(epoch):
        warmup = 2
        if epoch < warmup:
